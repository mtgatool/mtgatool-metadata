...
Toolkit.run(async tools => {
  ...
  let version = 'patch'
  if (messages.map(message => message.toLowerCase().includes('breaking')).includes(true)) {
    version = 'major'
  } else if (messages.map(message => message.toLowerCase().includes('feature')).includes(true)) {
    version = 'minor'
  }
  ...
  try {
    ...
    await tools.runInWorkspace('git', ['config', 'user.email', '"mtgatool@gmail.com"'])
