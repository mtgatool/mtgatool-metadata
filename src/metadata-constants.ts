...
export const SETS_DATA: Record<string, CardSet> = {
  ...
  // Sets added from Arena's own card database after sets.test.ts was
  // reconnected to CI. collation is -1 throughout: it is the MTGA booster id,
  // which stopped appearing in the logs, and a made-up value would be matched
...
export const DIGITAL_SETS = [
  ...
  "M19 Gift Pack",
  "Arena New Player Experience",
  // Same story, found by the mapping audit once these sets were added. Arena's
  // numbering does not match Scryfall's, and by number "Through the Omenpaths"
  // returned the WRONG card (Aunt May where Arena has Zora, Spider Fancier) —
  // no art is better than confident nonsense. Fallen Empires resolves by name
  // cleanly; Through the Omenpaths only partly, since Arena carries 199 cards
  // against Scryfall's 188. Magic Online Promos has a single card Scryfall does
  // not list under prm at all, so that one stays unresolved either way.
  "Fallen Empires",
  "Through the Omenpaths",
  "Magic Online Promos",
];
