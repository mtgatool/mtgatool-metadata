...
import { readSetFiles } from "./setFiles";
...
/**
 * Set data lives as one JSON file per set in sets/ (schema and caveats in
 * sets/README.md), so a new set is a new file — writable by the automated
 * update workflow and reviewable on its own — instead of a hunk in this one.
 */
export const SETS_DATA: Record<string, CardSet> = {};

/**
 * Names of sets the client resolves by NAME on Scryfall instead of by
 * collector number (the byName flag in each set's file). For Arena-only
 * digital sets and any set whose numbering does not match Scryfall's.
 */
export const DIGITAL_SETS: string[] = [];

readSetFiles().forEach((file) => {
  const { name, byName, ...set } = file;
  SETS_DATA[name] = set;
  if (byName) DIGITAL_SETS.push(name);
});
